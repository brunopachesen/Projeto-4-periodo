...
public class AutomovelDAO {
   public static void inserir(AutomovelBean automovel) throws SQLException{
        ...
        public static List<AutomovelBean> pesquisar() throws SQLException {
            ...
            Connection con = Conexao.getConexao();
            String sql = "select * from tb_modelo INNER JOIN  tb_automovel ON tb_modelo.id_mod = tb_automovel.id_mod\n" +
"     INNER JOIN  tb_fabrica ON tb_modelo.id_fab = tb_fabrica.id_fab";
            PreparedStatement stmt = con.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                automovel.getModelo().getFabricante().setNome(rs.getString("nome_fab"));
        ...
        public static List<AutomovelBean> pesquisar(AutomovelBean pesqAuto) throws SQLException {
            ...
            Connection con = Conexao.getConexao();
            String sql = "select * from tb_modelo INNER JOIN  tb_automovel ON tb_modelo.id_mod = tb_automovel.id_mod\n" +
"     INNER JOIN  tb_fabrica ON tb_modelo.id_fab = tb_fabrica.id_fab where chassi='"+pesqAuto.getChassi()+"'";
            PreparedStatement stmt = con.prepareStatement(sql);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                AutomovelBean automovel = new AutomovelBean();
               automovel.setModelo(new ModeloBean());
                ...
                automovel.setAno((rs.getInt("ano")));
                automovel.getModelo().setNome((rs.getString("nome_mod")));
                automovel.getModelo().getFabricante().setNome("nome_fab");
                listaAutomovel.add(automovel);
            }
            rs.close();
            stmt.close();
        ...
        public static AutomovelBean pesquisar(String chassi) throws SQLException {
            ...
            Connection con = Conexao.getConexao();
            String sql = "select * from tb_modelo INNER JOIN  tb_automovel ON tb_modelo.id_mod = tb_automovel.id_mod\n" +
"     INNER JOIN  tb_fabrica ON tb_modelo.id_fab = tb_fabrica.id_fab where chassi='"+chassi+"'";
            PreparedStatement stmt = con.prepareStatement(sql);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
            automovel.setIdAutomovel(rs.getInt("id_auto"));
